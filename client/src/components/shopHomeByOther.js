...
function shopHomeByOther() {
  ...
  const getUserIdFromItemId = () => {
    Axios.get("http://3.101.88.78:4000/getItemById/" + itemId).then(
      (response) => {
        if (response) {
  ...
  const getItemsFromUserid = () => {
    Axios.get(
      "http://3.101.88.78:4000/api/products/getAllProducts/" + userId
    ).then((response) => {
      if (response) {
  ...
  const getUserDetails = () => {
    Axios.get("http://3.101.88.78:4000/api/users/getShopById/" + userId).then(
      (response) => {
        if (response) {

...
const CartScreen = () => {
  ...
  const getCartList = () => {
    Axios.get(
      "http://54.151.45.102:4000/api/products/getCartItems/" + user.id
    ).then((response) => {
    ...
    if (user.about === null) {
      ...
      checkOutItems.map((product) => {
        if (product.qty === 0) {
          ...
          Axios.delete(
            "http://54.151.45.102:4000/api/products/deleteCartItem/" +
              product.itemId
          ).then((response) => {
          ...
          console.log(product);
          Axios.post(
            "http://54.151.45.102:4001/api/products/addProductToPurchase/",
            {
              product: product,
            }
          )
          ...
          Axios.put(
            "http://54.151.45.102:4000/api/products/editItemQtyById/" +
              product.itemId,
            itemDetails
          ).then((response) => {
      ...
      Axios.delete("http://54.151.45.102:4000/api/products/clearCart")
        .then((response) => {
          if (response) {
